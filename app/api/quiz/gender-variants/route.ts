...
// prompt. L'éditeur Tiquiz utilise du vrai HTML pour le formatage → ces
// marqueurs apparaissent EN CLAIR sur la page publique sinon (cf. bug
// Adeline 17 mai 2026 : "**Béné, tu es le Solopreneur..."). Defensive,
// idempotent, ne casse rien si l'input était déjà clean.
function stripMarkdown(s: string): string {
  return s
    // **bold** → bold (et __bold__)
    .replace(/\*\*([^*]+)\*\*/g, "$1")
    .replace(/__([^_]+)__/g, "$1")
    // *italic* → italic (mais on évite de matcher * isolés au milieu d'un mot
    // ou la mid-dot ·*. on cible \w*\w pour rester safe)
    .replace(/(^|[\s(.,;:!?«"'])\*([^*\s][^*]*?[^*\s]|[^*\s])\*(?=[\s).,;:!?»"']|$)/g, "$1$2")
    .replace(/(^|[\s(.,;:!?«"'])_([^_\s][^_]*?[^_\s]|[^_\s])_(?=[\s).,;:!?»"']|$)/g, "$1$2")
    // `code` → code
    .replace(/`([^`]+)`/g, "$1")
    // # heading → heading (en début de ligne uniquement)
    .replace(/^#{1,6}\s+/gm, "")
    .trim();
}
