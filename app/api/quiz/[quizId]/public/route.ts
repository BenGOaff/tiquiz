...
export async function GET(req: NextRequest, context: RouteContext) {
  try {
    ...
    const [quizRes, questionsRes, resultsRes] = await Promise.all([
      ...
      admin.from("quiz_results").select("id,title,description,insight,projection,insight_heading,projection_heading,cta_text,cta_url,sort_order,image_url,image_position").eq("quiz_id", quizId).order("sort_order"),
    ]);
