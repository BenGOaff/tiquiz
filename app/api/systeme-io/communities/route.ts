...
// GET user's Systeme.io communities. Accepts ?keyId= so the quiz editor
// can preview a specific key's communities.
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!resolved) return NextResponse.json({ ok: true, communities: [] });

    const res = await sioUserRequest<{ items: { id: number; name: string }[] }>(
      resolved.apiKey,
      "/community/communities?limit=100",
    );
