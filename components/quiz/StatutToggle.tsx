...
export default function StatutToggle({ actif, onToggle, libelles, disabled }: Props) {
  return (
    ...
    >
      ...
      <span
        aria-hidden
        className={`relative inline-block h-5 w-9 rounded-full transition-colors ${actif ? "bg-white/30" : "bg-foreground/15"}`}
      >
        <span
          className={`absolute top-0.5 h-4 w-4 rounded-full shadow transition-transform ${
            actif ? "translate-x-4 bg-white" : "translate-x-0.5 bg-background"
          }`}
        />
      </span>
      <span>{actif ? libelles.on : libelles.off}</span>
    </button>
  );
}
